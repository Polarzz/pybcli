...
def makeNormalVar(name,value):
	with open('/pyboard/os/path/'+name+".json",'w') as fw:
		f = """
{\"1\":\"2\"}
...
def makeVar(var,value):
	...
	if var == "$USER":
		...
		with open("/pyboard/os/env/"+var,'w') as f:
			f.write(listToStr(value.split()))
			f.close()
		return True
"""def getAllVars:
	return uos.listdir("/pyboard/os/env/")"""
def getUser():
	with open("/pyboard/os/path/USER","r") as f:
		ff = f.read()
	return ff.replace(" ","").replace("\n","")
COMMANDS = ["echo","led-toggle","exit","hard-reset","cd","ls","pwd","rmdir","mkdir","prompt","sw-status","free","rm","clear-cache","su","mkuser","set","del","./<file>","load"]
...
def getVar(name):
	try:
		...
		if name == "USER":
			...
			with open("/pyboard/os/env/"+name,"r") as f:
				ff = f.read()
			return ff.replace(" ","").replace("\n","")
	except:
		return " "
...
def execute(command,args):
	...
	if command == "mkuser":
		try:
			uos.mkdir("/pyboard/os/home/"+args)
	...
	if command == "su":
		try:
			for i in uos.listdir("/pyboard/os/home"):
				if i == args:
					with open("/pyboard/os/path/USER","w") as f:
						f.write(args)
						f.close()
						uos.chdir("/pyboard/os/home/"+getUser())
				#else:
	...
	if command == "del":
		try:
			...
			if args == "user":
				...
				uos.remove("/pyboard/os/env/"+args.replace("$","").upper())
		except:
			print("Failed.")
			error()
	if command == "clear":
